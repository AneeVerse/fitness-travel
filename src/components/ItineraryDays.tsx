...
const ItineraryDays: React.FC<ItineraryDaysProps> = ({ tripData }) => {
  ...
  const isPhuketFeb = tripData.slug === 'phuket' && tripData.dates?.toLowerCase().includes('15 feb');
                ...
                >
                  <div className="relative h-[360px] sm:h-[350px] md:h-[450px] lg:h-[520px] xl:h-[450px] rounded-2xl overflow-hidden shadow-xl bg-black group">
                    ...
                    <div className="relative w-full h-full">
                      ...
                      {/* Content */}
                      <div className={`absolute ${isPhuketFeb ? 'inset-3 sm:inset-4' : 'inset-4'} z-10 flex flex-col justify-between text-white select-none`}>
                        {/* Top Section */}
                        <div className="space-y-1 select-none">
                          <h3
                            className={`font-bold uppercase select-none whitespace-nowrap ${tripData.slug === 'phuket' ? 'text-xs sm:text-sm md:text-base lg:text-lg xl:text-xl scale-[0.6] sm:scale-100 origin-left' : 'text-sm sm:text-base md:text-xl lg:text-2xl'}`}
                        ...
                        <div className={`flex-1 flex flex-col justify-start space-y-2 select-none ${isPhuketFeb ? 'pt-2' : 'pt-5'}`}>
                          <h4 className={`${isPhuketFeb ? 'text-[10px] sm:text-xs md:text-sm' : 'text-[11px] sm:text-xs md:text-base'} font-semibold leading-tight select-none mb-2`}>
                            {day.title}
                          </h4>
                          ...
                          <ul className="space-y-1.5 select-none list-disc pl-4">
                            {(() => {
                              ...
                              const segments = day.description.includes('\n')
                                ? day.description.split(/\n+/)
                                : day.description.split(/,\s*/);
                              return segments.map((point, idx) => {
                                const trimmedPoint = point.trim();
                                if (!trimmedPoint) return null;
                                const baseText = isPhuketFeb ? 'text-[9px] sm:text-[11px] md:text-xs' : 'text-[10px] md:text-xs';
                                return (
                                  <li key={idx} className={`${baseText} opacity-90 leading-snug select-none`}>
                                    {trimmedPoint}
                                  </li>
                                );
                              });
                        ...
                        <div className="space-y-2 select-none">
                          {/* Details */}
                          <div className={`space-y-1.5 select-none ${isPhuketFeb ? 'text-[9px] sm:text-[11px] md:text-xs' : 'text-xs'}`}>
                            <div className="flex items-center gap-2 select-none">
                              <svg className="w-3 h-3 text-[#ef4a25]" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z" />
                              </svg>
                              <span className={`${isPhuketFeb ? 'text-[9px] sm:text-[11px] md:text-base' : 'text-[10px] md:text-base'} opacity-90 select-none`}>Check inclusions</span>
                            </div>
                            <div className="flex items-center gap-2 select-none">
                              <svg className="w-3 h-3 text-[#ef4a25]" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M12 2C8.134 2 5 5.134 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.866-3.134-7-7-7zm0 9.5a2.5 2.5 0 110-5 2.5 2.5 0 010 5z" />
                              </svg>
                              <span className={`${isPhuketFeb ? 'text-[9px] sm:text-[11px] md:text-base' : 'text-[10px] md:text-base'} opacity-90 select-none`}>{tripData.location}</span>
                            </div>
                            <div className="flex items-center gap-2 select-none">
                              <svg className="w-3 h-3 text-[#ef4a25]" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                              </svg>
                              <span className={`${isPhuketFeb ? 'text-[9px] sm:text-[11px] md:text-base' : 'text-[10px] md:text-base'} opacity-90 select-none`}>Professional Guided</span>
                            </div>
                          </div>

                          {/* Time Badge */}
                          <div className="flex justify-start select-none">
                            <div className={`bg-[#ef4a25] text-white px-2.5 py-1 rounded-full ${isPhuketFeb ? 'text-[9px] sm:text-[11px] md:text-xs' : 'text-[10px] md:text-xs'} font-semibold whitespace-nowrap select-none`}>
                              {day.time}
                            </div>
                          </div>
                        </div>
